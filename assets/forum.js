...
async function followingTopic(topicId){
  if(!session)return false;
  const {data}=await supabase.from("forum_topic_follows").select("topic_id").eq("user_id",session.user.id).eq("topic_id",topicId).maybeSingle();
  return !!data;
}
async function toggleFollow(topicId){
  if(!requireAuth())return;
  if(await followingTopic(topicId))await supabase.from("forum_topic_follows").delete().eq("user_id",session.user.id).eq("topic_id",topicId);
  else await supabase.from("forum_topic_follows").insert({user_id:session.user.id,topic_id:topicId});
  location.reload();
}
...
async function initTopic(){
  ...
  try{
    ...
    const [bmark,followed]=await Promise.all([bookmarked(id),followingTopic(id)]);
    ...
    $("[data-follow]")?.addEventListener("click",e=>toggleFollow(e.currentTarget.dataset.follow));
