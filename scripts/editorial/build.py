...
def journeys():
 body='<section class="sq-guides-hero"><span class="sq-section-index">Votre parcours</span><h1>Votre prochaine étape commence ici.</h1><p>Choisissez votre situation pour retrouver les guides utiles, dans le bon ordre, et avancer à votre rythme.</p><div class="sq-guides-tabs">'+''.join(f'<a href="#{key}">{escape(data[0])}</a>' for key,data in JOURNEYS.items())+'</div></section><div id="journeyMount" data-editorial-static="true">'
