...
for page in sorted(ROOT.rglob('*.html')):
    if any(x in {'.git','node_modules','test-results'} for x in page.relative_to(ROOT).parts): continue
    text=original=page.read_text(encoding='utf-8')
    if 'assets/sidebar.css' not in text or '</head>' not in text or '</body>' not in text: continue
    ...
    if 'data-sq-editorial-intelligence-style' not in text:text=text.replace('</head>',style+'\n</head>',1)
